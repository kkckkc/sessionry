...
export function PluginCard({
  ...
  const pluginInfo = isSearchResult(plugin) ? {
    ...
    author: typeof plugin.package.author === 'string' 
  ...
  return (
    <div className={`plugin-card plugin-card--${variant}`} data-plugin-id={pluginInfo.id}>
        ...
        <div className="plugin-card__actions">
          ...
          {variant === 'installed' && (
            <Button
              ...
              size="small"
              variant="danger"
              title={!pluginInfo.canUninstall ? 'Built-in plugins cannot be uninstalled' : undefined}
            >
              Uninstall
            </Button>
          )}
      ...
      {variant === 'available' && pluginInfo.validationWarnings && pluginInfo.validationWarnings.length > 0 && (
