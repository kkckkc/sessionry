...
import type { PluginSearchResult, PluginUpdateInfo } from '../../../main/npm/pluginManagerService';
...
export function PluginManagerProvider({ children }: PluginManagerProviderProps) {
  ...
  const updatePlugin = useCallback(
    async (pluginId: string, packageName: string) => {
      setOperation({
        operation: 'update',
        pluginId,
        status: 'loading',
    ...
    [refreshInstalledPlugins, checkForUpdates]
  );
  ...
  return <PluginManagerContext.Provider value={value}>{children}</PluginManagerContext.Provider>;
}
