    ...
    .reduce<NormalizedKeybindingState>(
      (state, segment) => {
        ...
        const normalizedSegment =
          lowered === 'c' || lowered === 'ctrl' || lowered === 'control' || lowered === 'cmd' || lowered === 'meta'
            ? 'C'
...
export const normalizeDeclaredKeybinding = (binding: string): string => {
  ...
  return [...modifierOrder.filter((modifier) => normalized.modifiers.includes(modifier)), normalized.key]
    .filter((part): part is string => Boolean(part))
    .join('-')
}

export const normalizeKeyboardEvent = (event: Pick<KeyboardEvent, 'key' | 'ctrlKey' | 'metaKey' | 'altKey' | 'shiftKey'>): string => {
...
export const createActionKeydownHandler = (
  ...
  for (const action of actions) {
    ...
    if (actionByKeybinding.has(normalizedKeybinding)) {
      console.warn(`[sessionry] Ignoring duplicate keybinding "${normalizedKeybinding}" for action "${action.id}".`)
      continue
    }
