...
const snapshot: WorkspaceStateSnapshot = {
  ...
  panes: [
    { id: 'pane-1', sessionId: 'session-1', type: 'terminal', preferredSizePct: 30, state: {} }
  ],
...
describe('createWorkspaceApi', () => {
  ...
  it('routes mutations through handle methods and root project creation', async () => {
    ...
    await workspace.projects[0].update({
      name: 'Updated',
      activeViews: { workspace: 'workspace.default' }
    });
    await workspace.projects[0].sessions[0].activate();
    await workspace.projects[0].sessions[0].setFocusedPane('pane-1');
    await workspace.projects[0].sessions[0].rootPaneGroup.update({
      activeChildId: 'pane-1',
      preferredSizePct: 80
    });
    ...
    expect(executeCommand).toHaveBeenNthCalledWith(6, {
      type: 'project.create',
