...
export function registerPluginManagerHandlers(
   ...
   * Install a plugin
   */
  ipcMain.handle('plugin:install', async (_event, options: { packageName: string; version?: string }) => {
    try {
   ...
   * Update a plugin
   */
  ipcMain.handle('plugin:update', async (_event, options: { pluginId: string; packageName: string }) => {
    try {
      const result = await pluginManagerService.updatePlugin(options.pluginId, options.packageName)
    ...
    try {
      ...
      const updateChecks = await Promise.allSettled(
        installedPlugins
          ...
          .map(plugin =>
            pluginManagerService.checkForUpdates(
              plugin.id,
              plugin.package!,
              plugin.version
            )
  ...
  ipcMain.handle('plugin:list', async () => {
    try {
      ...
      const builtinPluginList = builtInPlugins.map(plugin => ({
        ...
        installed: true,
        enabled: plugin.id === BUILTIN_CORE_PLUGIN_ID ? true : pluginConfigStore.isPluginEnabled(plugin.id),
