...
export class WorkspaceStore {
  ...
  read(): WorkspaceStateSnapshot {
    return {
      projects: this.state.projectOrder.map(projectId =>
        cloneValue(this.getProjectRequired(projectId))
      ),
      sessions: Array.from(this.state.sessions.values()).map(session => cloneValue(session)),
      paneGroups: Array.from(this.state.paneGroups.values()).map(paneGroup =>
        cloneValue(paneGroup)
      ),
  ...
  executeCommand(command: WorkspaceCommand): WorkspaceCommandResult {
    ...
    switch (command.type) {
      ...
      case 'session.setRootPaneGroup':
        result = {
          entityId: this.setSessionRootPaneGroup(command.sessionId, command.rootPaneGroupId).id
        };
        break;
  ...
  removeSession(sessionId: string): void {
    ...
    const nextActiveSessionId =
      this.state.activeSessionId === session.id
        ? this.getNextActiveSessionId(session)
  ...
  updatePaneGroup(paneGroupId: string, input: UpdatePaneGroupInput): PaneGroup {
  ...
  setPaneGroupChildren(paneGroupId: string, children: PaneGroupChild[]): PaneGroup {
    const paneGroup = this.getPaneGroupRequired(paneGroupId);
  ...
  insertPane(paneGroupId: string, paneId: string, index?: number): PaneGroup {
    ...
    const beforeChildren = cloneValue(paneGroup.children);
    paneGroup.children = this.insertChild(paneGroup.children, { kind: 'pane', paneId }, index);
  ...
  insertPaneGroup(paneGroupId: string, childPaneGroupId: string, index?: number): PaneGroup {
    const paneGroup = this.getPaneGroupRequired(paneGroupId);
  ...
  moveNode(node: PaneGroupChild, targetPaneGroupId: string, index?: number): PaneGroup {
    ...
    sourceParent.children = sourceParent.children.filter(
      child => paneNodeId(child) !== paneNodeId(node)
    );
  ...
  removeNode(node: PaneGroupChild): void {
      ...
      if (isRootGroup) {
        ...
        if (singleChild.kind === 'group') {
          ...
          promotedGroup.children.forEach((child, i) => {
            ...
            console.log(
              renderTree(child, isLast ? '   ' : '│  ').replace('├─', isLast ? '└─' : '├─')
            );
          });
        ...
        if (singleChild.kind === 'pane') {
          ...
          console.log(
            `Group: ${singleChild.paneGroupId} [${g?.direction}] (${g?.preferredSizePct ?? 0}%)`
          );
        }
        console.log('=== REMOVE NODE OPERATION END ===\n');
      } else {
    ...
    if (
      ...
      this.paneGroupContainsPane(node.paneGroupId, session.focusedPaneId)
    ) {
  ...
  splitPane(paneId: string, direction: 'horizontal' | 'vertical'): PaneGroup {
    ...
    const paneIndex = parentGroup.children.findIndex(
      child => isPaneChild(child) && child.paneId === paneId
    );
  ...
  splitPaneGroup(
    ...
    if (isRootGroup) {
      ...
      console.log(
        ...
        newChild
          ? newChild.kind === 'pane'
            ? `Pane: ${newChild.paneId}`
            : `Group: ${newChild.paneGroupId}`
          : 'New terminal pane'
      );
    ...
    console.log(
      'Target group:',
      paneGroupId,
      `[${paneGroup.direction}] (${paneGroup.preferredSizePct ?? 0}%)`
    );
    console.log(
      'New child:',
      newChild
  ...
  private emitChildrenChanged(paneGroup: PaneGroup, beforeChildren: PaneGroupChild[]): void {
    this.emit({
      type: 'paneGroup.childrenChanged',
      entityType: 'paneGroup',
      entityId: paneGroup.id,
    ...
    for (const paneGroup of this.state.paneGroups.values()) {
      if (
        paneGroup.children.some(
          child => child.kind === node.kind && paneNodeId(child) === paneNodeId(node)
        )
      ) {
        return paneGroup;
      }
  ...
  private getNextActiveSessionId(removedSession: Session): string | undefined {
    const sameProjectSessionId = removedSession.projectId
      ? this.getProjectRequired(removedSession.projectId).sessionIds.find(
          id => id !== removedSession.id
        )
      : undefined;

    if (sameProjectSessionId) return sameProjectSessionId;

    return Array.from(this.state.sessions.values()).find(
    ...
    const visitGroup = (paneGroupId: string): string | undefined => {
      ...
      const children =
        paneGroup.direction === 'stacked'
          ? paneGroup.children
              .filter(child => paneNodeId(child) === paneGroup.activeChildId)
              .slice(0, 1)
          : paneGroup.children;
  ...
  private getFallbackFocusedPaneId(
    sessionId: string,
    parent: PaneGroup | null
  ): string | undefined {
  ...
  private reconcileActiveChildAfterRemoval(
    ...
    if (removedIndex === -1) {
      paneGroup.activeChildId = paneGroup.children[0]
        ? paneNodeId(paneGroup.children[0])
        : undefined;
      return;
    }
  ...
  private determineSaveOpts(command: WorkspaceCommand): {
    workspace: boolean;
    ...
    switch (command.type) {
      ...
      case 'paneNode.remove': {
        if (command.node.kind === 'pane') {
          ...
          return {
            workspace: false,
            projectId: pane ? this.sessionProjectId(pane.sessionId) : null
          };
        }
