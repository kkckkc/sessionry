...
export class PluginManagerService extends EventEmitter {
  ...
  async searchPlugins(options: SearchOptions): Promise<PluginSearchResult[]> {
    ...
    try {
      const results = await this.registryClient.searchPlugins(options.query, {
        size: options.size,
        from: options.from
      });
      ...
      const enhancedResults: PluginSearchResult[] = await Promise.all(
        results.map(async result => {
          try {
            ...
            const metadata = await this.registryClient.getPluginMetadata(result.package.name);
            ...
            // Check compatibility
            const compatResult = this.validator.validateCompatibility(manifest as PluginManifest);
  ...
  async installPlugin(
    ...
    try {
      ...
      const metadata = await this.registryClient.getPluginMetadata(packageName, version);
      ...
      if (!options.skipValidation) {
        const validationResult = this.validator.validatePlugin(packageName, manifest, metadata);
  ...
  async getPluginInfo(
    ...
    const validation = this.validator.validatePlugin(packageName, manifest, metadata);
   ...
   */
  on<K extends keyof PluginManagerEvents>(event: K, listener: PluginManagerEvents[K]): this {
 ...
 */
export function createPluginManagerService(config?: PluginManagerConfig): PluginManagerService {
