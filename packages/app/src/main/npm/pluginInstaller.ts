...
export class PluginInstaller {
  ...
  async install(
    ...
    try {
      ...
      if (!options.skipValidation) {
        await this.verifyIntegrity(tarballPath, metadata.dist.shasum, metadata.dist.integrity);
      }
      ...
      https
        .get(url, response => {
          ...
          response.on('error', error => {
            ...
            reject(
              new InstallerError(`Download error: ${error.message}`, 'DOWNLOAD_FAILED', error)
            );
          });
          ...
          fileStream.on('error', error => {
            ...
            fs.unlink(filePath).catch(() => {});
            reject(
              new InstallerError(`File write error: ${error.message}`, 'DOWNLOAD_FAILED', error)
            );
          });
        ...
        .on('error', error => {
          reject(new InstallerError(`Network error: ${error.message}`, 'DOWNLOAD_FAILED', error));
        });
    });
  }
   ...
   */
  async verifyIntegrity(filePath: string, shasum?: string, integrity?: string): Promise<void> {
    if (!shasum && !integrity) {
      throw new InstallerError('No integrity checksums provided', 'MISSING_CHECKSUMS');
    }
    ...
    if (integrity) {
      ...
      if (!match) {
        throw new InstallerError(`Invalid integrity format: ${integrity}`, 'INVALID_INTEGRITY');
      }
   ...
   */
  async extract(tarballPath: string, destDir: string, pluginId: string): Promise<void> {
    try {
