...
const toDescriptor = ({ run: _run, ...action }: ActionContribution): ActionDescriptor => cloneValue(action)

const getNodeId = (child: PaneGroup['children'][number]): string =>
  child.kind === 'pane' ? child.paneId : child.paneGroupId

const getActivePaneId = (snapshot: WorkspaceStateSnapshot, sessionId?: string): string | undefined => {
...
export class ActionRegistry {
  ...
  async execute(request: ActionExecutionRequest): Promise<ActionExecutionResult> {
    ...
    const activeProjectId =
      snapshot.sessions.find((session) => session.id === activeSessionId)?.projectId ?? snapshot.projects[0]?.id
