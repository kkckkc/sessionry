...
describe('ActionRegistry', () => {
    ...
    expect(() =>
      createRegistry([
        ...
        },
        {
          ...
          name: 'Two',
          actions: [
            { id: 'duplicate', name: 'Duplicate Again', run: () => ({ status: 'completed' }) }
          ]
        }
      ])
  ...
  it('creates a usable terminal session from the built-in create session action', async () => {
    ...
    const rootPaneGroup = snapshot.paneGroups.find(
      paneGroup => paneGroup.id === createdSession?.rootPaneGroupId
    );
