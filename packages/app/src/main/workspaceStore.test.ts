...
describe('WorkspaceStore', () => {
  it('seeds a showcase workspace tree with horizontal, vertical, and stacked layouts', () => {
    ...
    expect(
      snapshot.paneGroups.find(paneGroup => paneGroup.id === 'pane-group-left-tabs')
    ).toMatchObject({
      direction: 'stacked',
    ...
    expect(
      snapshot.paneGroups.find(paneGroup => paneGroup.id === 'pane-group-secondary-root')
    ).toMatchObject({
      direction: 'stacked',
  ...
  it('moves nodes within a session and reconciles stacked active tabs on removal', () => {
    ...
    expect(service.getPaneGroup('pane-group-left-tabs')?.activeChildId).toBe(
      'pane-terminal-primary'
    );
  });
...
it('splits a pane in a non-matching direction group by creating a new group', () => {
  ...
  const firstPaneId =
    horizontalGroup?.children[0]?.kind === 'pane' ? horizontalGroup.children[0].paneId : null;
...
it('converts a pane to a stacked (tabbed) group', () => {
  ...
  const newPaneChild = result.children.find(
    c => c.kind === 'pane' && (c as any).paneId !== paneId && (c as any).paneId !== 'pane-activity'
  );
...
it('collapses horizontal/vertical groups with single child', () => {
  ...
  const promotedPane = finalLeftTabs.children.find(
    c => c.kind === 'pane' && (c as any).paneId === firstPane
  );
