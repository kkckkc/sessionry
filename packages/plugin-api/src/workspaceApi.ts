...
export interface WorkspaceBridge {
  ...
  executeCommand(command: WorkspaceCommand): Promise<WorkspaceCommandResult> | WorkspaceCommandResult
...
export const createWorkspaceApi = (bridge: WorkspaceBridge): WorkspaceApi => {
  ...
  const getProjectData = (id: string): ProjectData =>
    getRequired(read().projects.find((project) => project.id === id), 'Project', id)

  const getSessionData = (id: string): SessionData =>
    getRequired(read().sessions.find((session) => session.id === id), 'Session', id)

  const getPaneGroupData = (id: string): PaneGroupData =>
    getRequired(read().paneGroups.find((paneGroup) => paneGroup.id === id), 'Pane group', id)

  const getPaneData = (id: string): PaneData =>
    getRequired(read().panes.find((pane) => pane.id === id), 'Pane', id)
  ...
  class ProjectHandleImpl implements ProjectHandle {
    ...
    async createSession(
      input: Omit<CreateSessionInput, 'projectId'>
  ...
  class PaneGroupHandleImpl implements PaneGroupHandle {
    ...
    get children(): PaneNodeHandle[] {
      return this.data.children.map((child) =>
        isPaneGroup(child) ? new PaneGroupHandleImpl(child.paneGroupId) : new PaneHandleImpl(child.paneId)
      )
    }
    ...
    async setChildren(children: PaneGroupChild[]): Promise<void> {
      await bridge.executeCommand({ type: 'paneGroup.setChildren', paneGroupId: this.id, children })
    }

    async insertPane(paneId: string, index?: number): Promise<void> {
      await bridge.executeCommand({ type: 'paneGroup.insertPane', paneGroupId: this.id, paneId, index })
    }
    ...
    async moveNode(node: PaneGroupChild, index?: number): Promise<void> {
      await bridge.executeCommand({ type: 'paneNode.move', node, targetPaneGroupId: this.id, index })
    }
