...
export const PaneHierarchyView: React.FC<PaneHierarchyViewProps> = ({ workspace }) => {
  ...
  const activeSession = snapshot.activeSessionId
    ? // biome-ignore lint/suspicious/noExplicitAny: Debug plugin uses any for flexible snapshot inspection
  ...
  const renderNode = (child: PaneGroupChild, prefix: string, isLast: boolean): void => {
    ...
    const dirChar =
      group.direction === 'horizontal' ? 'H' : group.direction === 'vertical' ? 'V' : 'S';
  ...
  const rootDir =
    rootGroup.direction === 'horizontal' ? 'H' : rootGroup.direction === 'vertical' ? 'V' : 'S';
