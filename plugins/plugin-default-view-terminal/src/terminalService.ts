...
const resolveShellCandidates = (): string[] => {
  if (process.platform === 'win32') {
    return [process.env.COMSPEC, 'powershell.exe', 'cmd.exe'].filter((value): value is string =>
      Boolean(value)
    );
  }
...
export class TerminalService {
  ...
  constructor(
    ...
    if (this.tmuxBin) {
      console.log(
        `[sessionry] tmux found at "${this.tmuxBin}" — sessions will persist across restarts.`
      );
  ...
  createSession(input: CreateTerminalSessionInput): TerminalSessionInfo {
    ...
    const size = this.normalizeSize(input.cols, input.rows) ??
      existing?.size ??
    ...
    if (this.tmuxBin) {
      ...
      try {
        const pty = spawn(
          this.tmuxBin,
          [...this.tmuxBaseArgs(), 'attach-session', '-t', tmuxName],
          {
        ...
        pty.onExit(({ exitCode }) => {
          ...
          const sessionAlive =
            exitCode === 0 || this.tmuxSessionExists(toTmuxName(input.sessionId));
          if (sessionAlive) {
        ...
        console.warn(
          '[sessionry] Failed to start tmux session, falling back to direct shell.',
          error
        );
      }
    }
    ...
    throw new Error(
      `Unable to start a terminal shell. Tried: ${this.shellCandidates.join(', ') || 'none'}. ${reason}`
    );
  }
