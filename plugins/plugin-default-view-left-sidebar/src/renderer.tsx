...
const ProjectAvatar = ({
  ...
  return (
    ...
    >
      <span className="project-avatar" style={{ backgroundColor: color }}>
...
const countTerminalPanesInSession = (
  workspace: SidebarViewProps['workspace'],
  sessionId: string
): number => {
  const paneGroupById = new Map(
    workspace.snapshot.paneGroups.map(paneGroup => [paneGroup.id, paneGroup])
  );
...
const ProjectSessionsSidebarView = ({ workspace }: SidebarViewProps) => {
  ...
  useEffect(() => {
    ...
    const loadVcsStatuses = async () => {
      const nextEntries = await Promise.all(
        workspace.snapshot.sessions.map(
          async session =>
            [session.id, await window.terminalApp.vcs.getStatus(session.folder)] as const
        )
      );
  ...
  const openColorPicker = (event: React.MouseEvent<HTMLButtonElement>, project: Project) => {
  ...
  return (
    <>
        ...
        <ul className="projects-list" aria-label="Project sessions">
          {workspace.snapshot.projects.map(project => {
            const sessions = project.sessionIds
              .map(sessionId =>
                workspace.snapshot.sessions.find(session => session.id === sessionId)
              )
            ...
            return (
              <li key={project.id} className="project-item">
                ...
                >
                  <ProjectAvatar project={project} onClick={e => openColorPicker(e, project)} />
                  <span
                  ...
                  <div className="project-slot">
                    {/* biome-ignore lint/a11y/useAriaPropsSupportedByRole: Badge count with aria-label for screen readers */}
                ...
                <div
                  className={
                    isCollapsed ? 'sessions-list-wrapper is-collapsed' : 'sessions-list-wrapper'
                  }
                >
                      ...
                      return (
                        ...
                        >
                          <button
                            ...
                            onContextMenu={e =>
                              openContextMenu(e, 'session', session.id, session.name)
                            }
                          >
                            ...
                            {gitDiff && (gitDiff.insertions > 0 || gitDiff.deletions > 0) ? (
                              ...
                              >
                                ...
                                <span className="session-git-diff-remove">
                                  -{gitDiff.deletions}
                                </span>
                              </span>
                            ) : null}
      ...
      >
        <Menu.Portal>
          <Menu.Positioner className="menu-positioner" anchor={contextMenu?.anchor}>
